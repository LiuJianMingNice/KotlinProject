...
import com.example.jianshudemo.adapter.ShoeAdapter
import com.example.jianshudemo.common.listener.SimpleAnimation
import com.example.jianshudemo.databinding.FragmentShoeBinding
import com.example.jianshudemo.utils.UiUtils
import com.example.jianshudemo.viewmodel.CustomViewModelProvider
import com.example.jianshudemo.viewmodel.ShoeModel
import com.example.jianshudemo.widget.smartrefresh.DropBoxHeader
...
class ShoeFragment : Fragment() {
    ...
    private fun onSubscribeUi(binding: FragmentShoeBinding) {
        ...
        adapter.addLoadStateListener { state: CombinedLoadStates ->
            ...
            //如果append没有处于加载状态，但是refreshLayout处于加载状态，refreshLayout停止加载状态
            if (state.append is LoadState.NotLoading && binding.refreshLayout.isLoading) {
                refreshLayout.finishLoadMore()
            }
            //如果refresh没有处于在家状态，但是refreshLayout出于刷新状态，refreshLayout停止刷新
            if (state.append is LoadState.NotLoading && binding.refreshLayout.isLoading) {
        ...
        lifecycleScope.launch {
            //注意：这里需要用collectLatest，只用collect的话筛选会不生效
