package com.example.myapplication.mvvm.network

import com.amazing.eye.bean.CommentBean
import com.amazing.eye.bean.HotBean
import com.example.myapplication.mvvm.bean.FindBean
import com.example.myapplication.mvvm.bean.HomeBean
...
interface RestService {
    ...
    //获取发现详情加载更多消息
    @GET("v3/videos")
    fun getFindDetailMoreData(
        @Query("start") start: Int, @Query("num") num: Int,
        @Query("categoryName") categoryName: String, @Query("strategy") strategy: String
    ): Observable<HotBean>
    ...
    @GET("v1/search")
    fun getSearchData(
        @Query("num") num: Int, @Query("query") query: String,
    ...
    fun getCommentListData(
        @Query("num") num: Int, @Query("id") id: Int,
