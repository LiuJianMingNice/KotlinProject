 ...
 *
 * DirectionControlView(支持摇杆和按钮控制)
 * @author liujianming
 * @date 2022-02-09
 */
class DirectionControlView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isRocker) {
            ...
            mTopBitmap?.let { canvas.drawBitmap(it, (mCenterPoint!!.x - mTopBitmap!!.width / 2).toFloat(), 0f,  mRockerPaint) }
            mBottomBitmap?.let { canvas.drawBitmap(it, (mCenterPoint!!.x - mBottomBitmap!!.width / 2).toFloat(), (mCenterPoint!!.y - mBottomBitmap!!.height / 2).toFloat() * 2,  mRockerPaint) }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                Log.i(_tag, "onTouchEvent: 抬起位置 : x = $upX y = $upY")
                drawDirectionKeyBitmap(1, 1, 1, 1)
                invalidate()
            }
        }
