...
import com.example.myapplication.databinding.ActivityMainMediaStoreApiBinding
...
class MainMediaStoreApiActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val binding = ActivityMainMediaStoreApiBinding.inflate(layoutInflater)
        ...
        binding.pickFile.setOnClickListener {
            pickFileAndCopyUriToExternalFileDir()
        }
    ...
    private fun downloadFile(fileUrl: String, fileName: String) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            Toast.makeText(this, "You must use device running Android 10 higher", Toast.LENGTH_SHORT).show()
            return
        }
        thread {
            try {
    ...
    private fun pickFileAndCopyUriToExternalFileDir() {
        ...
        when (requestCode) {
            PICK_FILE -> {
                if (resultCode == Activity.RESULT_OK && data != null) {
                    ...
                    if (uri != null) {
                        ...
                        copyUriToExternalFileDir(uri, fileName)
                    }
                }
            }
            ...
            PICK_IMAGES -> {
                if (resultCode == Activity.RESULT_OK && data != null) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                        ...
                        startIntentSenderForResult(editPendingIntent.intentSender, CREATE_WRITE_REQUEST, null, 0, 0, 0)
                    } else {
            ...
            CREATE_WRITE_REQUEST -> {
                if (requestCode == Activity.RESULT_OK) {
                    Toast.makeText(this, "Write permission are granted", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(this, "Write permission are denied", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun copyUriToExternalFileDir(uri: Uri, fileName: String) {
        thread {
            ...
            if (inputStream != null && tempDir != null) {
                ...
                fos.close()
                runOnUiThread {
                    Toast.makeText(this, "Copy file into $tempDir succeeded.", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
